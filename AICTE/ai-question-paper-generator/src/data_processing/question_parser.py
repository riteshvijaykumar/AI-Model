...
Handles parsing of different question bank formats including CSV, Excel, JSON, TXT, and PDF files.
...
class QuestionParser:
    ...
    def parse_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        if file_extension not in self.supported_formats:
            ...
            raise ValueError(f"Unsupported file format: {file_extension}. Supported formats: {available_formats}")
        ...
        if file_extension == '.csv':
            ...
            raise ValueError(f"Parser for {file_extension} not implemented")
    ...
    def _parse_pdf(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        if not PDF_AVAILABLE:
            raise ImportError("PDF parsing requires PyPDF2 and pdfplumber. Install with: pip install PyPDF2 pdfplumber")
        ...
        try:
            ...
            try:
                ...
                questions = self._parse_text_to_questions(text)
                self.logger.info(f"Extracted {len(questions)} questions from PDF using fallback method")
                return questions
                
            except Exception as e2:
                self.logger.error(f"PDF parsing failed with both methods: {e2}")
    ...
    def _parse_text_to_questions(self, text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if line.endswith('?') or any(keyword in line.lower() for keyword in ['what', 'how', 'why', 'when', 'where', 'which', 'calculate', 'explain', 'describe']):
                if current_question:
            ...
            elif current_question and line.lower().startswith(('answer:', 'ans:', 'solution:')):
