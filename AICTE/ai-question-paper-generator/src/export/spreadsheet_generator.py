...
Generates formatted Excel, CSV, and PDF files from selected questions.
...
class SpreadsheetGenerator:
    """Generates formatted spreadsheets and PDFs from question data"""
    ...
    def generate_output(self, questions: List[Dict[str, Any]], 
                       output_path: str, 
                       format_type: str = 'excel',
                       columns: Optional[List[str]] = None,
                       marks_config: Optional[Dict[str, Any]] = None,
                       **kwargs) -> bool:
        """
        Generate output in specified format (Excel, CSV, PDF, JSON, or TXT)
        
        Args:
            questions: List of question dictionaries
            output_path: Path for output file
            format_type: 'excel', 'csv', 'pdf', 'json', or 'txt'
            columns: List of columns to include
            marks_config: Configuration for PDF marks distribution
        ...
        try:
            ...
            if format_type == 'pdf':
                return self.generate_pdf(questions, output_path, marks_config)
            elif format_type == 'json':
                return self.generate_json(questions, output_path, **kwargs)
            elif format_type == 'txt':
                return self.generate_txt(questions, output_path, **kwargs)
            else:
                return self.generate_spreadsheet(questions, output_path, format_type, columns, **kwargs)
        except Exception as e:
            self.logger.error(f"Error generating output: {e}")
            return False
    
    def generate_pdf(self, questions: List[Dict[str, Any]], 
                    output_path: str,
                    marks_config: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        """
        try:
            print(f"Debug: Starting PDF generation for {len(questions)} questions")
            print(f"Debug: Output path: {output_path}")
            print(f"Debug: Marks config: {marks_config}")
            
            result = self.pdf_manager.export_questions_to_pdf(
                questions, output_path, marks_config
            )
    ...
    def generate_json(self, questions: List[Dict[str, Any]], 
        ...
        try:
            ...
            export_data = {
                'metadata': {
                    ...
                    'format_version': '1.0',
                    'source': 'AI Question Bank Selection System'
                },
                'questions': questions
            }
    ...
    def generate_txt(self, questions: List[Dict[str, Any]], 
                    output_path: str, **kwargs) -> bool:
        """
        ...
        Args:
            ...
            **kwargs: Additional options (format_style: 'simple', 'detailed', 'exam')
        ...
        try:
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                # Write header
                f.write("AI Question Bank Selection System - Exported Questions\n")
                f.write("=" * 60 + "\n")
                f.write(f"Total Questions: {len(questions)}\n")
                f.write(f"Export Date: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write("=" * 60 + "\n\n")
                
                # Write questions based on format style
                for i, question in enumerate(questions, 1):
                    if format_style == 'simple':
                        f.write(f"{i}. {question.get('question', 'N/A')}\n\n")
                    
                    elif format_style == 'exam':
                        marks = question.get('marks', 2)
                        f.write(f"Q{i}. {question.get('question', 'N/A')} ({marks} marks)\n\n")
                        if question.get('answer'):
                            f.write(f"   Answer: {question.get('answer')}\n\n")
                    
                    else:  # detailed
                        f.write(f"Question {i}:\n")
                        f.write(f"  Text: {question.get('question', 'N/A')}\n")
                        f.write(f"  Topic: {question.get('topic', 'N/A')}\n")
                        f.write(f"  Difficulty: {question.get('difficulty', 'N/A')}\n")
                        f.write(f"  Type: {question.get('type', 'N/A')}\n")
                        f.write(f"  Marks: {question.get('marks', 'N/A')}\n")
                        if question.get('keywords'):
                            f.write(f"  Keywords: {question.get('keywords')}\n")
                        if question.get('answer'):
                            f.write(f"  Answer: {question.get('answer')}\n")
                        f.write("\n" + "-" * 40 + "\n\n")
            
            self.logger.info(f"TXT export completed: {output_path}")
            print(f"✅ TXT export successful: {output_path}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error generating TXT: {e}")
